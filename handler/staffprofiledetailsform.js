...
/*******************************************************************************
 * This class is responsible for the collection of the staff form submitted by an existing staff that want to upload his personal profile details online. The class return the congratulation = you personal info is now managed securely in the cloud.
 * *****************************************************************************
 */
...
class StaffProfileDetailsForm
{
    ...
    constructor(form) 
    {
        ...
        else 
        {
            ...
            /**********************************************
             * Check for user-code submitted with the form, it also shows that the top admin had previously created for this user to exist in the staff-table. Thereby making it secured for only authorized staffs to create an account.
             * **********************************************
             */
            if (!this.#form['application_type'].value.trim())
            {
                this.#_mErrors++;
                this.#mErrorMsg = "Your 20 digit user-code is required!";
                return;
            }
            else if(this.#form['application_type'].value.trim().length != 20)
            {
                this.#_mErrors++;
                this.#mErrorMsg = "Invalid user-code on length!";
                return;

            }
            else 
                this.#staffUserCode = this.#form['application_type'].value.trim();
            ...
            else if(!/^(application\/pdf)$/gi.test(this.#form['applicationletter'].files[0].type))
            {
                ...
                this.#_mErrors++;
                this.#mErrorMsg = "The letter should be in PDF format only!";
                return;
            }
            else if (this.#form['applicationletter'].files[0].size > 400000)
            {
                this.#_mErrors++;
                this.#mErrorMsg = "This letter is too large, please resize and try again!";
                return;
            }
            else 
            ...
            {
                this.#_mErrors++;
                this.#mErrorMsg = "PDF CV format only!";
                return;
            }
            ...
            else if (this.#form['alevelcertificate'].files[0].size > 400000)
            {
                ...
                {
                    // console.log(this.#form['attestation']);
                    this.#_mErrors++;
    ...
    processIt()
    {
        if(this.#_mErrors == 0)
        {
            preloader("Working, please wait...");
            
            // console.log(this.#staffEmpInfoObj);
            /**
             * Run the asynchronous code that uploads staff data to the firebase database and show DATA-UPLOAD-SUCCESSFULL screen.
             */

         
                
                switch (this.#form['application_class'].value) 
                {
                    case "FreshStaffApplication":
                        

                        /**
                         * Prepare the object that contains the display data and pass it to the class. This class is dynamically invoked.
                         */
                        ...
                        .catch(error => {
                            ...
                            toastIt('red', 'Unexpected error0!, please try again');
                        })
                        break;
                    ...
                    case "StaffProfileDetailsForm":
                        ...
                        .then(m => {
                            ...
                            let completedTask = new m.UserTaskCompletedUi({
                                pagetitle: `Upload <b>Successful!</b>`,
                                userwithsalutaion: `Dear Bangis`,
                    ...
                    default:
                        toastIt('red', 'Unexpected Error2! please try again.');
                        break;
                }
...
export { StaffProfileDetailsForm };